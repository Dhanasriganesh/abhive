...
// Load environment variables from root .env first, then fallback to src/.env
const rootEnvResult = dotenv.config();
if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
  dotenv.config({ path: 'src/.env' });
}

if (rootEnvResult.error) {
  console.warn('⚠️  Warning: Could not load root .env file:', rootEnvResult.error.message);
} else {
...
try {
  ...
  console.log('📄 root .env file found, size:', envContent.length, 'characters');
  if (envContent.length < 50) {
    console.warn('⚠️  root .env file seems very small - checking src/.env fallback');
  }
} catch (err) {
  console.error('❌ Cannot read root .env file:', err.message);
}

try {
  const srcEnvContent = readFileSync('src/.env', 'utf8');
  console.log('📄 src/.env file found, size:', srcEnvContent.length, 'characters');
} catch (err) {
  console.warn('⚠️  Cannot read src/.env fallback file:', err.message);
}
